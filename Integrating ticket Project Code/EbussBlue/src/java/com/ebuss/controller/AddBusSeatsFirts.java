...
import com.utils.IsertBusRoots;
import com.utils.UpdateBusSeatStatus;
 ...
 * @author Ramu
 */
public class AddBusSeatsFirts extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       String cdate = request.getParameter("cdate").trim();
        ...
        IsertBusRoots itr = new IsertBusRoots();
        try{
        int no =itr.addBussRootsSeats(cdate, trainno, trainname, sourcecity, destinationcity, dist, cost1, aSeats, aSeats);
        if(no>0){
        UpdateBusSeatStatus ubs = new UpdateBusSeatStatus();
        String msg = ubs.updateSeat(trainno, trainname);
        response.sendRedirect("AddBusSeats.jsp?msg="+msg);
        }
